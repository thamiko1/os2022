...
sys_open(void)
{
  ...
  if(ip->type == T_SYMLINK && !(omode & O_NOFOLLOW)){
    for(i=0; i<10 && ip->type == T_SYMLINK; i++){
      int pathlen = 0;
      if(readi(ip, 0, (uint64)&pathlen, 0, sizeof(int)) != sizeof(int))
        panic("open: readi");
      if(readi(ip, 0, (uint64)path, sizeof(int), pathlen+1) != pathlen+1)
        panic("open: readi");
...
sys_symlink(void)
{
  ...
  if(writei(ip, 0, (uint64)&pathlen, 0, sizeof(int)) != sizeof(int))
    panic("symlink: writei");
  if(writei(ip, 0, (uint64)target, sizeof(int), pathlen+1) != pathlen+1)
    panic("symlink: writei");
